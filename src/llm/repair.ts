...
// Sağlayıcı-agnostik retry/repair çekirdeği. Ollama + Claude (ve gelecekteki writer'lar) buradan
// import eder (DRY). SAF(ish): ağdan bağımsız → smoke'ta fake `generate` ile deterministik test edilir.
...
function parseJson(cleaned: string): unknown {
  try {
    ...
    if (!m) throw new Error(`JSON parse edilemedi: ${cleaned.slice(0, 200)}`);
...
// Sağlayıcı-agnostik: `generate` Ollama /api/chat çağrısı da olabilir, Anthropic Messages çağrısı da.
