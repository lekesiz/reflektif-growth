...
export async function bulkJson<T>(opts: {
  loop: string;
  jobId?: number;
  system: string;
  user: string;
...
export async function writerJson<T>(opts: {
  loop: string;
  jobId?: number;
  system: string;
  user: string;
  ...
  if (vertexConfigured()) {
    try {
      const out = await vertexJson<T>({ system: opts.system, user: opts.user, schema: opts.schema });
